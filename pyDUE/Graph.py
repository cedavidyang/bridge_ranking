...
class Graph:
    ...
    def add_link(self, startnode, endnode, route=1, flow=0.0, delay=0.0, ffdelay=0.0, delayfunc=None,
            capacity=None, length=None, freespeed=None):
        """Add a link"""
        ...
        if (startnode, endnode, route) in self.links:
            ...
            link = Link(startnode, endnode, route, float(flow), float(delay), float(ffdelay), delayfunc, capacity, length, freespeed)
    ...
    def add_links_from_list(self, list, delaytype):
        ...
        """
        for startnode, endnode, route, ffdelay, parameters,capacity,length,freespeed in list:
            self.add_link(startnode, endnode, route, 0.0, ffdelay, ffdelay,
                    delayfunc=create_delayfunc(delaytype, parameters), capacity=capacity,
                    length=length, freespeed=freespeed)


    def modify_link(self, startnode, endnode, route=1, flow=0.0, delay=0.0, ffdelay=0.0, delayfunc=None, 
            capacity=None, length=None, freespeed=None):
        """ modify existing link """
        # check if exist
        if self.links[(startnode, endnode, route)] is None:
            logging.error('link ({},{},{}) doest not exist'.format(startnode, endnode, route))
            assert('link ({},{},{}) doest not exist'.format(startnode, endnode, route))
        else:
            link = Link(startnode, endnode, route, float(flow), float(delay), float(ffdelay), delayfunc, capacity, length, freespeed)
            self.links[(startnode, endnode, route)] = link
            if delayfunc is not None:
    ...
    def modify_links_from_lists(self, list, delaytype):
        """Modify links from list"""
        for startnode, endnode, route, ffdelay, parameters,capacity,length,freespeed in list:
            self.modify_link(startnode, endnode, route, 0.0, ffdelay, ffdelay,
                    create_delayfunc(delaytype, parameters), capacity, length, freespeed)
